...
upload.prototype.init = function (ext){
    ...
    var option = {
        ...
        init: {
            ...
            FilesAdded: function(up, files) {
                var max_files = 3;
                var tuiguangtupian = $("#imgUrl").val();
                var getsum = [];
                var getsum = tuiguangtupian.split(",");
                var count = getsum.length;
                plupload.each(files, function(file) {
                    if (count+1 > max_files) {
                        alert('你只能上传 ' + max_files + ' 张图片.');
                        up.removeFile(file);
                    }
                });
    ...
    $(document).on('click', '#'+this.containerobj.attr('imgid')+' .delimg' ,function(e){
        ...
        ajax_post(site_url+"/upload/delphoto",{filename:filepath},function(data){
